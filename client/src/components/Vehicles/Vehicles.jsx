...
const Vehicles = () => {
    ...
    const fun = async () => {
        ...
        const vehicleList = list.map(vehicle => {
            ...
            <div className="Vehicles_Unit" key={vehicle.id}>
                ...
                <img src={vehicle.photo} alt="Vehiculo" className="Vehicles_photo"></img>
                <p>{city}</p>
                <p className="Vehicles_Text2">Disponibilidad:</p>
                {vehicle.availability ?
                    <p style={{color: "green"}} className="Vehicles_Text2">Disponible</p>:
                    <p style={{color: "red"}} className="Vehicles_Text2">No disponible</p>
                    }
                    {
                        vehicle.availability ?
                        <button className="Vehicles_Button" onClick={statusChangeHandler} value={[vehicle.availability, vehicle.id]}>Desabilitar</button>:
                        <button className="Vehicles_Button" onClick={statusChangeHandler} value={[vehicle.availability, vehicle.id]}>Habilitar</button>
                    }
            </div>
        )});
    ...
    const statusChangeHandler = async (event) => {
        ...
        console.log(state);
        await axios.post(`/vehicles/${state[1]}`, {availability : (state[0] === "true" ? false : true)})
        fun()
    };
    ...
    return(
        <div className="Vehicles_Main">
            <div  className="Vehicles_Second">
                Lista de vehiculos:
                <div className="Vehicles_Unit_Box">
                    {vehiclesListLocal}
                </div>
            </div>
        </div>
